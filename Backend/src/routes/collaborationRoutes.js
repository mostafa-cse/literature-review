...
router.get('/projects/:id/members', (req, res) => {
  ...
  try {
    ...
    const members = db.prepare(`
      SELECT pm.id as membership_id, pm.role as project_role, pm.created_at as joined_at,
...
router.post('/projects/:id/members', authenticateToken, (req, res) => {
  ...
  try {
    ...
    logAuditEvent(req, 'COLLABORATOR_ADD', `Added ${cleanEmail} as ${validRole} on project ${projectId}`, 'SUCCESS');

    res.status(201).json({
      message: `Added ${targetUser.name} (${cleanEmail}) as ${validRole} successfully.`,
...
router.put('/projects/:id/members/:userId', authenticateToken, (req, res) => {
  ...
  const validRole = ['owner', 'editor', 'reviewer', 'viewer'].includes(role) ? role : 'editor';
