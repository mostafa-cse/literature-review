  ...
  try {
    ...
    if (format === 'bib' || format === 'bibtex') {
      const bibEntries = papers.map(p => {
        ...
        bib += `  publisher = {LitSphere Research Matrix}\n`;
        bib += `}\n`;
        return bib;
      }).join('\n');
      ...
      return res.send(`% LitSphere Systematic Literature Review - BibTeX Export\n% Project: ${proj ? proj.name : 'Master Matrix'}\n% Total Citations: ${papers.length}\n\n${bibEntries}`);
    }
    ...
    if (format === 'tex' || format === 'latex') {
      ...
      let tex = `% Camera-Ready Literature Survey Matrix (Generated by LitSphere)\n`;
      tex += `\\begin{table*}[t]\n`;
      tex += `\\centering\n`;
