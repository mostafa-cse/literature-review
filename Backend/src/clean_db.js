...
function cleanDatabase() {
  console.log('🧹 [LitNexis DB Clean] Starting standard database reset with seed accounts...');
  initDb();
  const db = getDb();
  ...
  try {
    ...
    insertUser.run(1, 'admin', 'System Administrator', 'admin@litnexis.ac', adminHash, 'admin', 'LitNexis Academic Engineering', 'active', 500000, 0, 2048, 0.0);
    insertUser.run(2, 'researcher', 'Lead Researcher', 'researcher@litnexis.ac', researcherHash, 'user', 'Academic Research Institute', 'active', 100000, 0, 500, 0.0);
    insertUser.run(3, 'coauthor', 'Dr. Sarah Chen (Co-Author)', 'coauthor@litnexis.ac', coauthorHash, 'user', 'AI & Machine Learning Lab', 'active', 120000, 0, 800, 0.0);
    insertUser.run(4, 'advisor', 'Prof. Robert Vance (Advisor)', 'advisor@litnexis.ac', advisorHash, 'supervisor', 'Graduate Faculty of Engineering', 'active', 300000, 0, 2000, 0.0);

    // 3. Ensure System Settings are clean
    db.exec(`
      INSERT OR REPLACE INTO system_settings (key, value) VALUES ('maintenance_mode', 'false');
      INSERT OR REPLACE INTO system_settings (key, value) VALUES ('maintenance_message', 'LitNexis Platform is currently undergoing scheduled maintenance.');
    ...
    console.log('✅ [LitNexis DB Clean] Database reset with 4 standard seed accounts and 0 surveys.');
  } catch (err) {
    db.exec('ROLLBACK;');
    db.exec('PRAGMA foreign_keys = ON;');
...
function wipeAllUsersAndData() {
  console.log('🧹 [LitNexis DB Wipe] Starting complete wipe (0 users, 0 surveys)...');
  initDb();
  const db = getDb();
  ...
  try {
    ...
    console.log('✅ [LitNexis DB Wipe] Database is now 100% completely empty!');
